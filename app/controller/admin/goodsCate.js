...
class GoodsCateController extends BaseController {
  async index(){
    let list = await this.service.goodsCateDbService.showCateList();
    await this.ctx.render('admin/goodsCate/index',{list});
  }
  async add() {
    let list=await this.service.goodsCateDbService.findByCondition({pid:'0'});
    await this.ctx.render('admin/goodsCate/add',{list});
  }
  async doAdd(){
...
// console.log(stream);
      ...
      let result=await this.service.goodsCateDbService.add(Object.assign(files,fields));
      if(result){
          await this.success('/admin/goodsCate','添加商品分类成功');
      }
  }
    //修改商品分类首页
    async edit(){
        ...
        //console.log(id);
        let cate = await this.service.goodsCateDbService.findOne({_id:id});
        //console.log(cate);
        let list = await this.service.goodsCateDbService.findByCondition({pid:'0'});
        await this.ctx.render('admin/goodsCate/edit',{cate,list});
...
// console.log(stream);
        ...
        let result=await this.service.goodsCateDbService.updateOne({_id:id},Object.assign(files,fields));
        if(result){
            await this.success('/admin/goodsCate','修改商品成功');
        }
    }
